...
def arg_parser():
  ...
  parser.add_argument('--log_file', type=str,
                      default='logs/mc_RAPIQUE_LinearSVR_test_0.03.log',
                      help='Log files.')
  ...
  parser.add_argument('--num_iterations', type=int, default=11,
...
def evaluate_bvqa_one_split(i, X, y, log_short):
  ...
  X_train, X_test, y_train, y_test = \
    train_test_split(X, y, test_size=0.03, random_state=math.ceil(8.8*i))
  ...
  if X_train.shape[1] > 4000:
    ...
    # grid = RandomizedSearchCV(SVR(), param_grid, cv=9, n_jobs=-1)
  else:
...
def main(args):
  ...
  if args.dataset_name == 'LIVE_VQC':
      ...
      y = array[1:,4]
  elif args.dataset_name == 'mc_study':
      y = array[1:,9]
  ...
  X = imp.transform(X)
  # X = X[:, 1360:3408]
  # np.savetxt("feats_mc.csv", X, delimiter=",")
  ...
  if args.use_parallel is True:
    ...
    for i in range(1, args.num_iterations):  # 重複の検証
...
python evaluate_bvqa_features_regression.py \
  ...
  --out_file result/KONIQ_10K_BRISQUE_SVR_corr.mat (correlation)\
  --use_parallel
