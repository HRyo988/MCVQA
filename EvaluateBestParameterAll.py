...
def arg_parser():
    ...
    parser.add_argument('--log_file', type=str,
                        default='logs/McRAPIQUELinearSVRAllUsingBestParam.log',
                        help='Log files.')
...
def compute_metrics(y_pred, y, flag):
  ...
  '''
    ...
    if flag:
        ...
        plt.plot(Xrange, logistic_func(Xrange, *popt), color='r')
        plt.xlabel('Predicted Score')
        plt.ylabel('MOS')
        plt.xlim(right=5.5, left=0.5)
        plt.ylim(top=5.5, bottom=0.5)
        plt.savefig('mc_study_all.jpg')
...
def evaluate_bvqa_one_split(i, X, y, log_short):
    ...
    if not log_short:
        t_end = time.time()
        formatted_print(metrics_train + metrics_test,(t_end - t_start))
...
def main(args):
    ...
    for i in range(0, args.num_iterations):
        metrics_train, metrics_test = evaluate_bvqa_one_split(i, X_del, y_del, args.log_short)
